...
$(document).ready(function () {
	...
	$(document).on("click", ".w_ajax_link", doAjaxLink);
});

function showLoadingPage() {
	$('#i_loading_overlay').show();
}

function hideLoadingPage() {
	setTimeout(function () {
		$("#i_loading_overlay").hide();
	}, 200);
}

function doAjaxLink(e, source_obj) {
	e.preventDefault();
	...
	var data = obj.attr("data-ajax");

	if (data) {
		data = JSON.parse(obj.attr("data-ajax"));
	}

	if (obj.hasClass("w_ajax_confirm")) {
		var msg = obj.attr('data-msg');
		if (msg === undefined || msg === '') {
			msg = "Dữ liệu không thể khôi phục sau khi làm thao tác này! <br><b>Bạn chắc chắn sẽ làm điều này chứ?</b>";
		}
		...
		swal({
			...
			confirmButtonClass: 'waves-effect waves_primary_color c_btn ez_red c_mar_left_10',
			cancelButtonClass: 'waves-effect c_btn_outline ez_gray'
		}).then(function () {
			callAjaxLink(url, data, obj);
		}, function () {
		});
	}
	else {
...
function callAjaxLink(url, data, obj) {
	showLoadingPage();
	...
	$.ajax({
		...
		success: function (data) {
			if (window[data.callback]) {
				...
				console.log("Callback function not found:'", data.callback, "'-->Call 'responseAjaxLinkDefault' instead");

				responseAjaxLinkDefault(data, obj);
			}
		},
		error: function () {
			//window.location = url;
...
function responseExportOrder(data, obj) {
	...
	if (data.state === 1) {
		...
		$("body").append($a);
		$a.attr("download", "danh-sach-don-hang.xls");
		$a[0].click();
		$a.remove();
	}
	else {
...
function responseExportProduct(data, obj) {
	obj.removeClass('disabled');
	...
	if (data.state === 1) {
		...
		$a.attr("href", data.file);
		$("body").append($a);
		$a.attr("download", "danh-sach-san-pham.xls");
		$a[0].click();
		$a.remove();
	}
	else {
